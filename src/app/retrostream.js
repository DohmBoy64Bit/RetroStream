import { installCarousels } from './carousels.js';
import { browsePath, legacyHashToUrl, parseRoute, titlePath, watchPath, withParams } from './routing.js';
import { collectionSchema, DEFAULT_DESCRIPTION, DEFAULT_ROBOTS, mediaSchema, setPageSeo, webSiteSchema } from './seo.js';
const $=s=>document.querySelector(s);
const main=$('#main');
const masthead=document.querySelector('.masthead');
const mobileMenu=$('#mobile-menu-toggle');
function closeMobileMenu(focus=false){if(!masthead||!mobileMenu)return;masthead.classList.remove('menu-open');mobileMenu.setAttribute('aria-expanded','false');mobileMenu.setAttribute('aria-label','Open menu');if(focus)mobileMenu.focus();}
if(mobileMenu){mobileMenu.onclick=()=>{const open=!masthead.classList.contains('menu-open');masthead.classList.toggle('menu-open',open);mobileMenu.setAttribute('aria-expanded',String(open));mobileMenu.setAttribute('aria-label',open?'Close menu':'Open menu');};masthead.querySelector('nav')?.addEventListener('click',e=>{if(e.target.closest('a'))closeMobileMenu();});}
function navigate(url,{replace=false}={}){const next=new URL(url,location.origin);if(next.origin!==location.origin){location.href=next.href;return;}closeMobileMenu();history[replace?'replaceState':'pushState'](null,'',next.pathname+next.search);route();}
function migrateLegacyRoute(){const legacy=legacyHashToUrl(location.hash);if(legacy){history.replaceState(null,'',legacy);return;}if(location.pathname.length>1&&location.pathname.endsWith('/'))history.replaceState(null,'',location.pathname.replace(/\/+$/,'')+location.search);}
function applyRouteSeo(parts){const kind=parts[0];if(kind==='home'){setPageSeo({title:'RetroStream — Discover Movies & TV Series',description:DEFAULT_DESCRIPTION,canonicalPath:'/',schema:webSiteSchema()});return;}if(kind==='browse'){const isTv=parts[1]==='tv';const pageTitle=isTv?'TV Series — Popular Shows & New Releases | RetroStream':'Movies — Popular Films & New Releases | RetroStream';const description=isTv?'Browse popular TV series, discover new shows, filter by genre, year, and rating, and explore cast and episode details on RetroStream.':'Browse popular movies, discover new releases, filter by genre, year, and rating, and explore cast and movie details on RetroStream.';const path=browsePath(parts[1]);setPageSeo({title:pageTitle,description,canonicalPath:path,schema:collectionSchema(isTv?'TV Series':'Movies',description,path)});return;}if(kind==='latest'){const description='Discover new and recent movie and TV releases, then explore ratings, cast, details, and available episodes on RetroStream.';setPageSeo({title:'New & Recent Movies and TV | RetroStream',description,canonicalPath:'/new',schema:collectionSchema('New & Recent',description,'/new')});return;}if(kind==='search'){setPageSeo({title:'Search Movies & TV Series | RetroStream',description:'Search the RetroStream catalog by movie title, TV series, or IMDb ID.',canonicalPath:'/search',robots:'noindex, follow, max-image-preview:large'});return;}if(kind==='watch'){setPageSeo({title:'Watch on RetroStream',description:'RetroStream playback page.',canonicalPath:location.pathname,robots:'noindex, follow'});return;}if(kind==='title'){setPageSeo({title:`${parts[1]==='tv'?'TV Series':'Movie'} Details | RetroStream`,description:DEFAULT_DESCRIPTION,canonicalPath:location.pathname});}}
document.addEventListener('click',e=>{if(e.defaultPrevented||e.button!==0||e.metaKey||e.ctrlKey||e.shiftKey||e.altKey)return;const a=e.target.closest('a[href]');if(!a||a.target||a.hasAttribute('download'))return;const raw=a.getAttribute('href');if(!raw||raw.startsWith('#'))return;const url=new URL(a.href,location.href);if(url.origin!==location.origin)return;e.preventDefault();navigate(url.pathname+url.search);});
...
const cache=new Map();
async function api(path,params={}){const url='/api/tmdb?'+new URLSearchParams({path,...params});if(cache.has(url))return cache.get(url);const r=await fetch(url);const d=await r.json();if(!r.ok)throw Error(d.error||'Could not load the catalog.');cache.set(url,d);return d;}
...
function cards(items){return items.filter(m=>type(m)!=='person'&&!m.adult).map(m=>`<a class="poster-card" href="${titlePath(type(m),m.id,name(m))}" aria-label="${esc(name(m))}, ${date(m)}, ${type(m)==='tv'?'TV series':'movie'}"><div class="poster-wrap">${photo(m.poster_path,name(m))}<span class="poster-score">${score(m)}</span><span class="poster-type">${type(m)==='tv'?'SERIES':'FILM'}</span></div><h3>${esc(name(m))}</h3><p><span>${date(m)||'Release TBA'}</span><span>${esc((genres[type(m)]||[]).find(g=>m.genre_ids?.includes(g.id))?.name||'')}</span></p></a>`).join('');}
...
async function watch(t,id,s=1,e=1,g){const m=await api(t+'/'+id,{append_to_response:'external_ids'});let season;if(t==='tv')season=await api(`tv/${id}/season/${s}`);if(g!==generation)return;const watchCanonical=watchPath(t,id,s,e);const watchImage=m.backdrop_path?img(m.backdrop_path,'w1280'):m.poster_path?img(m.poster_path,'w780'):'';setPageSeo({title:`Watch ${name(m)}${t==='tv'?` — S${Number(s)} E${Number(e)}`:''} | RetroStream`,description:m.overview||`Watch ${name(m)} on RetroStream.`,canonicalPath:watchCanonical,robots:'noindex, follow',image:watchImage,imageAlt:`${name(m)} artwork`,type:'video.other'});const imdb=m.imdb_id||m.external_ids?.imdb_id;const episode=season?.episodes.find(x=>x.episode_number===Number(e));const valid=t==='movie'||(episode?.air_date&&episode.air_date<=new Date().toISOString().slice(0,10));const src=imdb&&/^tt\d+$/.test(imdb)?`https://vidsrc2.ru/embed/${t}/${imdb}${t==='tv'?'/'+Number(s)+'/'+Number(e):''}?autoplay=0`:null;main.innerHTML=`<div class="content"><a class="back-link" href="${titlePath(t,id,name(m))}">&larr; Back to title details</a><div class="watch-head"><div><h1>${esc(name(m))}</h1>${episode?`<p>Season ${Number(s)} · Episode ${Number(e)} — ${esc(episode.name)}</p>`:''}</div>${t==='tv'?`<div class="actions"><a class="btn btn-secondary" href="${titlePath('tv',id,name(m))}?episodes=1">All episodes</a>${season.episodes.some(x=>x.episode_number===Number(e)+1&&x.air_date&&x.air_date<=new Date().toISOString().slice(0,10))?`<a class="btn btn-primary" href="${watchPath('tv',id,s,Number(e)+1)}">Next episode &rarr;</a>`:''}</div>`:''}</div>${src&&valid?`<iframe class="player" src="${src}" title="Watch ${esc(name(m))}${episode?' — '+esc(episode.name):''}" allow="autoplay; encrypted-media; fullscreen; picture-in-picture" allowfullscreen referrerpolicy="origin"></iframe><p class="player-note">Player not loading? <button class="link" id="reload-player">Reload player</button>. Playback is supplied by VidSrc and may include ads. Availability varies.</p>`:`<div class="error-box"><h2>Playback isn’t available for this title yet.</h2><p>${!valid?'This episode has not aired or could not be found.':'No IMDb identifier is available for this title.'}</p><a class="btn btn-secondary" href="${titlePath(t,id,name(m))}">Back to details</a></div>`}<p>${esc(episode?.overview||m.overview)}</p></div>`;if($('#reload-player'))$('#reload-player').onclick=()=>{const f=$('.player');f.src=src;};}
async function route(){const g=++generation;const parsed=parseRoute(location.pathname,location.search),parts=parsed.parts,p=parsed.params;if(!parts){history.replaceState(null,'','/');return route();}applyRouteSeo(parts);const activeNav=parts[0]==='browse'?parts[1]:parts[0];document.querySelectorAll('[data-nav]').forEach(a=>{const active=a.dataset.nav===activeNav;a.classList.toggle('active',active);if(active)a.setAttribute('aria-current','page');else a.removeAttribute('aria-current');});main.innerHTML=loading();window.scrollTo(0,0);try{if(parts[0]==='home')await home(g);else if(parts[0]==='search')await browse(p.get('type')||'all',p,g,true);else if(parts[0]==='latest')await browse(p.get('type')||'movie',p,g,false,true);else if(parts[0]==='browse'&&['movie','tv'].includes(parts[1]))await browse(parts[1],p,g);else if(['title','watch'].includes(parts[0])&&['movie','tv'].includes(parts[1])&&/^\d+$/.test(parts[2])){if(parts[0]==='title')await title(parts[1],parts[2],p,g);else await watch(parts[1],parts[2],parts[3]||1,parts[4]||1,g);}}catch(e){if(g===generation){const robots=document.head.querySelector('meta[name="robots"]')?.content||DEFAULT_ROBOTS;setPageSeo({title:'Content unavailable | RetroStream',description:'This RetroStream page could not be loaded right now.',canonicalPath:location.pathname,robots});main.innerHTML=`<div class="content">${errorHtml(e)}</div>`;main.querySelector('[data-retry]').onclick=route;}}}
main.addEventListener('click',e=>{const b=e.target.closest('[data-feature]');if(b){featureIndex=Number(b.dataset.feature);$('#featured').innerHTML=feature(featured[featureIndex]);}});
$('#search-launch').onclick=()=>{closeMobileMenu();if(location.pathname==='/search')$('#query')?.focus();else navigate('/search');};document.addEventListener('keydown',e=>{if(e.key==='Escape'&&masthead?.classList.contains('menu-open')){e.preventDefault();closeMobileMenu(true);return;}if(e.key==='/'&&!['INPUT','TEXTAREA','SELECT'].includes(document.activeElement.tagName)){e.preventDefault();$('#search-launch').click();}});window.addEventListener('popstate',route);
async function boot(){migrateLegacyRoute();await Promise.allSettled([api('configuration').then(c=>imageBase=c.images.secure_base_url),...['movie','tv'].map(t=>api('genre/'+t+'/list').then(d=>genres[t]=d.genres))]);route();}boot();
if(document.modelContext?.registerTool){const lifecycle=new AbortController();window.addEventListener('pagehide',()=>lifecycle.abort(),{once:true});try{Promise.resolve(document.modelContext.registerTool({name:'search_retrostream_catalog',description:'Search the movie and series catalog and show the matching results.',inputSchema:{type:'object',properties:{query:{type:'string',minLength:1,maxLength:180}},required:['query'],additionalProperties:false},annotations:{readOnlyHint:false,untrustedContentHint:true},async execute(input){if(!input||typeof input.query!=='string'||!input.query.trim()||input.query.length>180)throw Error('Enter a title or IMDb ID.');const target=withParams('/search',{q:input.query.trim(),type:'all'});history.pushState(null,'',target);await route();return {query:input.query.trim(),summary:$('#summary')?.textContent||'Catalog could not be loaded.'};}},{signal:lifecycle.signal})).catch(()=>{});}catch{}}
