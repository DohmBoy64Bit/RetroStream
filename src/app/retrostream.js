...
import { collectionSchema, DEFAULT_DESCRIPTION, mediaSchema, setPageSeo, webSiteSchema } from './seo.js';
...
async function route(){const g=++generation;const parsed=parseRoute(location.pathname,location.search),parts=parsed.parts,p=parsed.params;if(!parts){history.replaceState(null,'','/');return route();}applyRouteSeo(parts);const activeNav=parts[0]==='browse'?parts[1]:parts[0];document.querySelectorAll('[data-nav]').forEach(a=>{const active=a.dataset.nav===activeNav;a.classList.toggle('active',active);if(active)a.setAttribute('aria-current','page');else a.removeAttribute('aria-current');});main.innerHTML=loading();window.scrollTo(0,0);try{if(parts[0]==='home')await home(g);else if(parts[0]==='search')await browse(p.get('type')||'all',p,g,true);else if(parts[0]==='latest')await browse(p.get('type')||'movie',p,g,false,true);else if(parts[0]==='browse'&&['movie','tv'].includes(parts[1]))await browse(parts[1],p,g);else if(['title','watch'].includes(parts[0])&&['movie','tv'].includes(parts[1])&&/^\d+$/.test(parts[2])){if(parts[0]==='title')await title(parts[1],parts[2],p,g);else await watch(parts[1],parts[2],parts[3]||1,parts[4]||1,g);}}catch(e){if(g===generation){setPageSeo({title:'Content unavailable | RetroStream',description:'This RetroStream page could not be loaded right now.',canonicalPath:location.pathname,robots:'noindex, follow'});main.innerHTML=`<div class="content">${errorHtml(e)}</div>`;main.querySelector('[data-retry]').onclick=route;}}}
